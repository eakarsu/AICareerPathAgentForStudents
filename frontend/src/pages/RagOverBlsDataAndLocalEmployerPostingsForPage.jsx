...
// Feature: RAG over BLS data and local employer postings for personalized labor-market insights
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverBlsDataAndLocalEmployerPostingsForPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-bls-data-and-local-employer-postings-for-/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over BLS data and local employer postings for personalized labor-market insights</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-bls-data-and-local-employer-postings-for-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
