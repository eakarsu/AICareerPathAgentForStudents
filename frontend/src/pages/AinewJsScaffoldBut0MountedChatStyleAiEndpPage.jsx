...
// Feature: aiNew.js scaffold but 0 mounted /chat-style AI endpoints
import React, { useState } from 'react';
import api from '../services/api';

export default function AinewJsScaffoldBut0MountedChatStyleAiEndpPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-ainew-js-scaffold-but-0-mounted-chat-style-ai-endp/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>aiNew.js scaffold but 0 mounted /chat-style AI endpoints</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-ainew-js-scaffold-but-0-mounted-chat-style-ai-endp/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
