...
// Feature: Agentic career coach guiding multi-week journey with weekly check-ins
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticCareerCoachGuidingMultiWeekJourneyWiPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-career-coach-guiding-multi-week-journey-wi/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic career coach guiding multi-week journey with weekly check-ins</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-career-coach-guiding-multi-week-journey-wi/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
